...
<div class="alert alert-warning text-center" style="margin: 10px 50px;">
   در اینجا میتوانید به عنوان طراح کاتالوگ ثبت نام کنید
</div>

<form action="" method="post" class="was-validated col-lg-4" style="margin-bottom: 50px;position: absolute;right:33%;direction: rtl;float: right !important;background-color:aliceblue;border-radius: 5px;padding: 20px " enctype="multipart/form-data">
<div class="form-group">
        <label  style="float: right;">نام:</label>
        <input type="text" name="name" class="form-control border-primary" placeholder="نام خود را به صورت لاتین وارد کنید">

    </div>
    <div class="form-group">
        <label style="float: right;">نام خانوادگی:</label>
        <input type="text" name="lastname" class="form-control border-primary"  placeholder="نام خانوادگی خود را وارد کنید">
...
-->
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-control border-primary" placeholder="نام کاربری خود را وارد کنید" name="username" required>
        <div class="valid-feedback">معتبر است.</div>
        <div class="invalid-feedback" style="text-align: right;margin-right: 20px;">لطفا فیلد مورد نظر را پر کنید.</div>
    </div>
    <div class="form-group">
        <label style="float: right;">رمز عبور:</label>
        <input type="password" class="form-control border-primary"  placeholder="رمز عبور خود را وارد کنید" name="password" required>
        <div class="valid-feedback">معتبر است.</div>
        <div class="invalid-feedback" style="text-align: right;margin-right: 20px;">لطفا فیلد مورد نظر را پر کنید.</div>
    </div>
    <div class="form-group">
        <label for="email" style="float: right;">ایمیل:</label>
        <input type="email" class="form-control border-primary" placeholder="لطفا ایمیل معتبر وارد کنید" name="email" required>
        <div class="valid-feedback">معتبر است.</div>
        <div class="invalid-feedback" style="text-align: right;margin-right: 20px;">لطفا فیلد مورد نظر را پر کنید.</div>
    </div>
    <div class="form-group">
        <label for="phone" style="float: right;">تلفن همراه:</label>
        <input type="number" class="form-control border-primary" name="phone" required>
    </div>

    <div class="form-group">
        <label for="degree" style="float: right;">مدرک تحصیلی</label>
        <div class="form-group">
            <select name="major" class="form-control border-primary">
...
if (isset($_POST['submit'])) {
    ...
    $sql = "SELECT email and username from tarahcata  WHERE email=? or username=?";
    ...
    if($result > 0)
    {
        ...
        $sql = ("INSERT INTO tarahcata(name , lastname ,username,password,email,phone,major)VALUES (?,?,?,?,?,?,?)");
        $query = $database->prepare($sql);
