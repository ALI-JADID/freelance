...
<h4 style="text-align: center;margin: 40px">پوستر</h4>

</div>

<form action="" method="post" class="was-validated col-lg-4" style="margin-bottom: 50px;position: absolute;right:33%;direction: rtl;float: right !important;background-color:azure;border-radius: 5px;padding: 20px " enctype="multipart/form-data">
...
<div class="invalid-feedback" style="text-align: right;margin-right: 20px;">لطفا فیلد مورد نظر را پر کنید.</div>
   ...
   <div class="form-group">
<label for="sel1" style="float: right;">زمان نگه داری پوستر در سایت بر حسب ساعت:</label>
<div class="form-group">
...
if (isset($_POST['submit'])) {
    ...
    $sql  = ("INSERT INTO `poster` (`username`,`email`,`phone`,`code`,`time`,`content`) VALUES  (?,?,?,?,?,?)");
//$result = $query->fetch(PDO::FETCH_ASSOC);
    $query = $database->prepare($sql);
    $result = $query->execute([$username,$email,$phone,$code,$time,$content]);
    if ($database) {
        if ($result) {
            echo "<script>alert('سفارش  با موفقیت ثبت شد')</script>";
        } else {
