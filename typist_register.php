...
<head>
    ...
    <title>ثبت نام تایپیست</title>
</head>
<body>

<div class="alert alert-success text-center" style="margin: 10px 50px;">
    در این جا میتوانید به عنوان تایپیست ثبت نام کنید
</div>

<form action="" method="post" class="was-validated col-lg-4" style="position: absolute;right:33%;direction: rtl;float: right !important;background-color: #c6fcf9;border-radius: 5px;padding: 20px ">
<div class="form-group">
        <label for="firstname" style="float: right;">نام:</label>
        <input type="text" class="form-control" name="name" placeholder="نام خود را به صورت لاتین وارد کنید">
    ...
    <div class="form-group">
        <label for="password" style="float: right;">رمز عبور:</label>
...
if (isset($_POST['submit'])) {
    ...
    $sql = "SELECT (email and username) from typist WHERE (email=? or username=?)";
    ...
    if($result >0)
    {
        ...
        $sql = "INSERT INTO typist(name,lastname,username,password,email,phone,major) VALUES (? , ? , ? , ? , ? ,? ,?)";
        $query = $database->prepare($sql);
